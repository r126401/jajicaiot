...
void accionTemporizada(DATOS_APLICACION *datosApp) {
    ...
    tiempo = (datosApp->programacion[programaActual].programa + 
                datosApp->programacion[programaActual].duracion ) -
                datosApp->clock.time;

    printf("accionTemporizada--> La duracion propuesta es de %d\n", datosApp->programacion[programaActual].duracion);
    
    if (datosApp->programacion[programaActual].duracion > 0)  {
        ...
        if (tiempo > 0 ) {
            ...
            if (tiempo >= INTERVALO) {
                ...
                printf("accionTemporizada--> La duracion es de %d\n", temporizacion);
                os_timer_disarm(&timerDuracion);
                os_timer_setfn(&timerDuracion, (os_timer_func_t*)accionTemporizada, datosApp);
                os_timer_arm(&timerDuracion, temporizacion*1000, false);
                ...
                printf("accionTemporizada--> La duracion es de %d\n", temporizacion);
                os_timer_disarm(&timerDuracion);
                os_timer_setfn(&timerDuracion, (os_timer_func_t*)appUser_ejecucionAccionTemporizada, datosApp);
                os_timer_arm(&timerDuracion, temporizacion*1000, false);

            }
        } else {
            printf("accionTemporizada--> Ya paso el intervalo de duracion\n");
            appUser_ejecucionAccionTemporizada(datosApp);
        }
    }
    
}

int ajustarProgramacion(DATOS_APLICACION *datosApp) {
    
    bool res;
    int actual= -1;
    //int futuro = -1;
    printf("ajustarProgramacion--> inicio\n");
    ordenarListaProgramas(datosApp->programacion, datosApp->nProgramacion, &(datosApp->clock));
    printf("ajustarProgramacion-->voy a buscar programa\n");
    res = buscarPrograma(datosApp, &actual );
    printf("ajustarProgramacion-->he buscado el programa\n");
    printf("ajustarProgramacion: actual %d, futuro %u\n", actual, datosApp->nProgramaCandidato);
    ...
    return datosApp->nProgramaCandidato;
...
//        memcpy(&reloj.date, gmtime(&reloj.time), sizeof(struct tm));
 ...
 void relojTiempoReal(NTP_CLOCK *clock) {
     ...
     static uint8 intervaloLectura=0;
     ...
     if(clock->timeValid == TRUE)  {
         ...
         if (datosApp.programacion == NULL) {
             //printf("relojTiempoReal-->No hay programacion, estadoApp: %d, estadoProgramacion: %d\n", datosApp.estadoApp, datosApp.estadoProgramacion);
             ...
             switch(datosApp.estadoProgramacion) {
                 case INVALID_PROG:
                     ...
                     printf("relojTiempoReal-->uptime: %u\n", system_get_rtc_time());
                     //programaFuturo = ajusteProgramacion(&datosApp, ARRANQUE);
                     programaFuturo = ajustarProgramacion(&datosApp);
                     datosApp.estadoApp = NORMAL_AUTO;
                 } else {
                     DBG("relojTiempoReal--> Estado normal sin programacion");
                     datosApp.estadoApp = NORMAL_SIN_PROGRAMACION;
                 }
                     break;
                 ...
                 case VALID_PROG:
                     ...
                     if (datosApp.estadoApp == NORMAL_ARRANCANDO) {
                         //programaFuturo = ajusteProgramacion(&datosApp, ARRANQUE);
                         programaFuturo = ajustarProgramacion(&datosApp);
                         datosApp.estadoApp = NORMAL_AUTO;
                     }
             ...
             //printf("relojTiempoReal: futuro %d, estadoApp: %d, estadoProgramacion: %d\n", datosApp.nProgramaCandidato, datosApp.estadoApp, datosApp.estadoProgramacion);
             //printf("relojTiempoReal-->estadoApp: %d, estadoProgramacion: %d, programa futuro:%d \n", datosApp.estadoApp, datosApp.estadoProgramacion, programaFuturo);
             if ((datosApp.programacion[datosApp.nProgramaCandidato].programa - datosApp.clock.time) == 0) {
                 printf("relojTiempoReal-->llegamos al temporizador y ejecutamos la accion que proceda\n");
                 //programaFuturo = ajusteProgramacion(&datosApp, CAMBIO_PROGRAMA);
                 programaFuturo = ajustarProgramacion(&datosApp);
                 printf("relojTiempoReal-->despues del ajuste estadoApp: %d, estadoProgramacion: %d, programa futuro:% \n", datosApp.estadoApp, datosApp.estadoProgramacion, programaFuturo);
             ...
             if((clock->date.tm_hour == 0) && (clock->date.tm_min == 0) && (clock->date.tm_sec == 0)) {
                 ...
                 //programaFuturo = ajusteProgramacion(&datosApp, MEDIA_NOCHE);
                 programaFuturo = ajustarProgramacion(&datosApp);
     ...
     //Acciones dependiendo del estado del reloj.
     switch(clock->estado) {
         case SIN_HORA:
             //Solo debe pasar en el arranque del dispositivo.
