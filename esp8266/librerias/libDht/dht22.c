...
bool DHTRead16(uint8 *data)
{
	...
	// wait for pin to drop?
	while (gpio16_input_get() == 1 && i < DHT_MAXCOUNT) {
		os_delay_us(1);
		i++;
	}
	...
	if(i == DHT_MAXCOUNT)
	{
		DHT_DEBUG("DHT16: Failed to get reading from GPIO%d, dying\r\n", 16);
	    return false;
	}
	...
	for (i = 0; i < DHT_MAXTIMINGS; i++)
	{
		// Count high time (in approx us)
		counter = 0;
		while (gpio16_input_get() == laststate)
		{
			counter++;
			os_delay_us(1);
			if (counter == 1000)
