...
#include "spi_register.h" //this must be included before "esp_common.h", because we need to use this more complete version (in standard spi_register.h some things are missing)
 ...
 */
	...
	Buf[Idx++] = 0x02;
        uint32 pepe=0;
	pepe = wlcd_write(FLG_SEND_CMD|FLG_PULSE_CS_ON_START, Cmd, (uint32_t*)Buf, Idx);
	//
        DBG_WLCD("wlcd_init-->Escritos %d datos\n",pepe);
	Idx = 0;
	Cmd = 0xCF;
	Buf[Idx++] = 0x00;
	Buf[Idx++] = 0xC1;
...
bool wlcd_status() {
    ...
    uint32_t STORE_ATTR Val32;
    uint32_t res=0;
    res = wlcd_read(FLG_SEND_CMD|FLG_PULSE_CS_ON_START, WLCD_READ_DISP_STATUS, 1, &Val32, 4);

    if  (( ((uint8_t*)&Val32)[0] == 0XFF ) &&
            ( ((uint8_t*)&Val32)[0] == 0XFF ) &&
            ( ((uint8_t*)&Val32)[0] == 0XFF ) &&
            ( ((uint8_t*)&Val32)[0] == 0XFF ) ||
            res == 0) {
        return false;
...
void wlcd_sleep() {
    ...
    //wlcd_cmd_only(WLCD_ENTER_SLEEP);
    //vTaskDelay(120 / portTICK_RATE_MS); //([ms] / portTICK_RATE_MS)
	//
    wlcd_cmd_only(WLCD_DISP_OFF);
    vTaskDelay(120 / portTICK_RATE_MS); //([ms] / portTICK_RATE_MS)
